  ...
  while (repetir_proceso) {
    ...
    for (variable in variables) {
      ...
      modelo_temporal <- vglm(as.formula(formula_nueva),
                              frechet(location = 0, lscale = "loglink",
                                      lshape = "loglink"),
                              trace = FALSE, data = dat,coefstart = rep(0, (length(variables_restantes) + 1)*2))
...
output_dir = paste0("MC_study_p5/MC_study_VGAM_AIC_n", n, "/")
...
for (iter_beta in 1: nrow(list_betas)){ 
  ...
  for(iter_mc in 1:n_mc){
    ...
    save(modelo_final, file = paste0(output_dir, "result_", paste(list_p_activos[iter_beta,], collapse = ""), "_", iter_mc, ".Rdata"))
...
load("MC_study_p5/MC_study_VGAM_AIC_n200/Matrix_n200.Rdata")
matrix_result_n200 = matrix_result
load("MC_study_p5/MC_study_VGAM_AIC_n400/Matrix_n400.Rdata")
matrix_result_n400 = matrix_result
load("MC_study_p5/MC_study_VGAM_AIC_n800/Matrix_n800.Rdata")

...
step_AIC <- function(dat, variables_iniciales = NULL){
  ...
  modelo_actual <- vglm(as.formula(formula_ini),
                        gpd(threshold = 0, lscale = "loglink",
                                lshape = "loglink", zero = NULL, type.fitted = "mean"),
...
for (n in c(200, 400, 800)){
  ...
  output_dir_AIC = paste0("../Example_Pareto_Generalized_scale/MC_study_p5/MC_study_VGAM_AIC_n", n, "/")
  if (!dir.exists(output_dir_AIC)) dir.create(output_dir_AIC, recursive = TRUE)
  output_dir_BIC = paste0("../Example_Pareto_Generalized_scale/MC_study_p5/MC_study_VGAM_BIC_n", n, "/")
  ...
  for (iter_beta in 1: nrow(list_betas)){ 
    ...
    for(iter_mc in 1:n_mc){
      ...
      set.seed(iter_mc)
      y = as.vector(rgpd(n = n, location = mu_true,  scale = sigma_true, shape = xi_true))
      y[y<0.1] = 0.1
      dat = data.frame(
        y = y
      )
...
for (n in c(200,400,800)){
  ...
  output_dir = paste0("../Example_Pareto_Generalized_scale/MC_study_p5/MC_study_VGAM_BIC_n", n , "/")
  matrix_result = array(0, dim  = c(nrow(list_betas), nrow(list_betas)))
  ...
  for (iter_beta in 1:nrow(list_betas)){
    ...
    for (iter_mc in 1:n_mc){
      ...
      matrix_result[iter_beta, which(apply(list_p_activos, 1, function(x) all(x == vec)))] = matrix_result[iter_beta, which(apply(list_p_activos, 1, function(x) all(x == vec)))] + 1/n_mc
      
    }
    matrix_result[iter_beta, ]
  }
  matrix_result = matrix_result[orden, rev(orden)]
  rownames(matrix_result) <- nombres
  colnames(matrix_result) <- rev(nombres)
...
load("../Example_Pareto_Generalized_scale/MC_study_p5/MC_study_VGAM_AIC_n200/Matrix_n200.Rdata")
matrix_result_n200 = matrix_result
load("../Example_Pareto_Generalized_scale/MC_study_p5/MC_study_VGAM_AIC_n400/Matrix_n400.Rdata")
matrix_result_n400 = matrix_result
load("../Example_Pareto_Generalized_scale/MC_study_p5/MC_study_VGAM_AIC_n800/Matrix_n800.Rdata")
...
final_plot
path_save = "../Example_Pareto_Generalized_scale/MC_study_p5/Img_Study/"
ggsave(filename = paste(path_save,"Pixel_plot_VGAM_AIC", ".png", sep= ""), plot = final_plot,
